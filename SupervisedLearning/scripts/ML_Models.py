...
#%% ++++++++++++++++++++++++ DNN model for Bws ++++++++++++++++++++++++++++++++
RetrainNNmodel = False
SaveNNModel = False
HyperParameterTuning = False
dot_img_file = dirpath + '/MODELS/model_1.png'
dnn_model = dirpath + '/MODELS/dnn_model'
if RetrainNNmodel:
    X_train, X_test, Y_train, Y_test, EN_train, EN_test \
        = mlnnf.TrainTestSplit(df,xfeatures,yfeatures,YfeaturesScale=100,frac=0.8)
    #sns.pairplot(X_train[['ANTIMONY','STRAIN']], diag_kind='kde')
    print("Summary of the training dataset:\n", X_train.describe().transpose())
    normalizer = mlnnf.FeatureNormalization(X_train)
    if HyperParameterTuning:
        model, history, metrices = mlnnf.OptimalHyperParameters(X_train,X_test,Y_train,Y_test)
    else:
        HyperParameters = mlnnf.CreateHyperParamers(len(X_train))
        nepochs,DECAY_STEP = HyperParameters[0], HyperParameters[3]
        model, history, metrices = mlnnf.compile_and_fit(normalizer, X_train, Y_train, 
                                                         DECAY_STEP, name=None, optimizer=None, 
                                                         lossfn='mean_squared_error',
                                                         max_epochs=nepochs)
    
    _ = mlpf.PlotMatrices(metrices, history)
...
assert isinstance(Ytest, np.ndarray)
mlpf.PlotTruePredictionCorrelation(Ytest, test_predictions, ttext, nrows=3)
mlpf.PlotTruePredictionErrorDistribution(Ytest, test_predictions, ttext, nrows=3)

#%%% ---------- evaluate model on bandgap nature performance ------------------
predicted_BWs = pd.DataFrame(test_predictions, columns=yfeatures)
predicted_BWs = mlgf.ConvertBWs2BandgapNature(predicted_BWs, BinaryConversion,colname='nature')

m = tf.keras.metrics.Accuracy(name='Accuracy')
m.update_state(EN_test.reset_index(drop=True),predicted_BWs['nature'])
print(f"The accuracy score for prediction: {m.result().numpy():.3f}")
...
_ = mlpf.DrawSnapshot(EqmDf,fname=movdirname+WhichSubstrate+'_sub.png', savefig=1, scale=100,
                      ...
                      vmin=emin, vmax=emax,titletext=f'Substrate: {WhichSubstrate}') 
...
for i in range(TotalSnapShot):#TotalSnapShot
    ...
    #mlpf.Plot3Ddecision_function(Xp,Yp,Zval,scale=100,TernaryConversion=True,hplane=False)
...
if DrawHtmls:
    ...
    #%%% ----------------------- Draw Bandgap Nature contours ---------------------
    #%%%%................ Draw all contours together in html ......................
    # fname = webdirname + 'StaticAllContour.html'
    # _ = mlwpf.DrawAllContourWeb(CONTOURS, StrainArray, fname=fname, 
    ...
    # fname = webdirname + 'SliderAllContour.html'
    # _ = mlwpf.DrawAllContourWebSlider(cnt, StrainArray, 
    #                                   savefig=1, scale=100, vmin=StrainMin, vmax=StrainMax,fname=fname,
    #                                   step=10,text=AxisLabels,cbarlabel=cbarlabel)
    #222222222222222222222222222222222222222222222222222222222222222222222222222222
    # fname = webdirname + 'SliderAllContourV2.html'
    # _ = mlwpf.DrawAllContourWebSliderV2([cnt, cnt, anticnt], StrainArray, CoverPage=CONTOURS,
    #                                     ContourText=['D','I'],
    #                                     IntializeContourText='', 
    #                                     savefig=1, scale=100, vmin=StrainMin, vmax=StrainMax,fname=fname,
    #                                     step=10,text=AxisLabels,cbarlabel=cbarlabel)
    #%%%%.................... Draw all contours with select in html ...............
    # fname = webdirname + 'SingleSelectAllContour.html'
    # _ = mlwpf.DrawAllContourWebSelect(cnt, StrainArray,
    #                                   savefig=1, scale=100, vmin=StrainMin, vmax=StrainMax,fname=fname,
    #                                   step=10,text=AxisLabels,cbarlabel=cbarlabel)
    #%%%%........ Draw all contours with Multi Select in html......................
    # fname = webdirname + 'MultiSelectAllContour.html'
    # _ = mlwpf.DrawAllContourWebMultiSelect(cnt, StrainArray, 
    #                                        savefig=1, scale=100, vmin=StrainMin, vmax=StrainMax,fname=fname,
    #                                        step=10,text=AxisLabels,cbarlabel=cbarlabel)
    #222222222222222222222222222222222222222222222222222222222222222222222222222222
    # fname = webdirname + 'MultiSelectAllContourV2.html'
    # _ = mlwpf.DrawAllContourWebMultiSelectV2([cnt, cnt, anticnt], StrainArray, CoverPage=CONTOURS,
    #                                          ContourText=['D','I'],
    #                                          IntializeContourText='',
    #                                          savefig=1, scale=100, vmin=StrainMin, vmax=StrainMax,fname=fname,
    #                                          step=10,text=AxisLabels,cbarlabel=cbarlabel)
    #%%%%............... Merge multiple html layouts ..............................
    # fname = webdirname + 'MergeLayoutsAllContour.html'
    # sliderlayout = mlwpf.DrawAllContourWebSlider(cnt, StrainArray,
    #                                              scale=100, vmin=StrainMin, vmax=StrainMax,
    #                                              step=10,text=AxisLabels,cbarlabel=cbarlabel)
    # multiselectlayout = mlwpf.DrawAllContourWebMultiSelect(cnt, StrainArray,
    #                                                        scale=100, vmin=StrainMin, vmax=StrainMax,
    #                                                        step=10,text=AxisLabels,cbarlabel=cbarlabel)
    
    # layoutlist=[sliderlayout,multiselectlayout]
    # mlwpf.MergeSliderSelect(layoutlist,fname=fname)
    #222222222222222222222222222222222222222222222222222222222222222222222222222222
    ...
    # data = pd.DataFrame({'X': x, 'Y': y, 'Z': bandgap})
    # data_pivoted = data.pivot("X", "Y", "X")
    # ax = sns.heatmap(data_pivoted,  square=True,  cmap="YlGnBu")
    # plt.show()