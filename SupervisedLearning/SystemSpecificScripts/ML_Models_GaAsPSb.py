...
OnlyGoodData = 0
UseLatticeParamterModel = False # <-- Training is based on LATTICEPARAMETER1: Check mlmf.SVMModelTrainingFunctions(). Model learns & predicts in-plane lattice parameter only.
DataCuration = False # <== Create good data

#### Model training
retrain_models = 1
StartFromScratch = True # <== This will delete the whole folder and start from scratch
SVM_Use_GridSearchCV = False # GridSearchCV vs RandomizedSearchCV
TOTALTRAINING = {'retraineg':True,
                 'retrainnature':True}

#### Plotting parameters
Plot_Ternary_Figs = True #<== Plot ternary plots. Require 'ternary' package
...
#%% ------------------------- Load database -----------------------------------
if OnlyGoodData:
    dbname = dirpath+'/DATAbase/Total_BPD_MLDataBase_GaAsPSb_GoodData.db'
else:
    dbname = dirpath+'/DATAbase/Total_BPD_MLDataBase_GaAsPSb.db'

df, points = mlgf.CreateDataForModel(dbname, ReturnPredictPoints=False,BinaryConversion=BinaryConversion,
                                     NatureMap={2: 1, 3: 0, 4: 0}) # If text {'direct': 1, 'indirect': 0}
...
TESTS = {'BT':False, 'BT_ct':False, 'BTq':False, 'Q':False, 'Q_ct':False, 'Qbt':False, 'BTQ':False,'BTq_ct':False,'Qbt_ct':False, 'BPD':True, 'BPD_lp':False}
PerformAll = False
if PerformAll: 
...
for XYZ in ['BTQ']:# ,'BTq_ct','Qbt_ct']:
    ...
    for fname in outnamelist:
        ...
        _ = mlgpf.PlotPostProcessingDataSetSizeLogLog(OutdataFrame,save=True,savepath=SaveFigPathTmp,figformat='eps')
        _ = mlgpf.PlotPostProcessingDataSetSizeLogLog(OutdataFrame,save=True,savepath=SaveFigPathTmp,figformat='png')
...
XP =mlmf.PlotConfusionMatrix(df['NATURE'], POINTS['EgN'], display_labels,
...
if Plot_Ternary_Figs:
    ...
    for SubstrateName in subname:
        ...
        if UseLatticeParamterModel or UseVegardsLaw:
            ...
            plt.savefig(f"{SubstrateEffect}/{SubstrateName}_sub_strain.eps",bbox_inches = 'tight',dpi=300)
            plt.close()
        else:
            EqmDf_ = None
            EqmDf_best = None
    ...
    #### Temporary to plot the bandgap nature and DIT lines
    # BandGapNatureHeatmap = 1
    # OnlyContours = 0
    # if BandGapNatureHeatmap:
    #     # tmp_featureseg = ["PHOSPHORUS",'ANTIMONY','ARSENIC','STRAIN','EgN_best_model']
    ...
    #                               generatetitle=1, savefig=0, axislabels=axislabels,
    ...
    #                               UseContoursText=False, ContoursText=[cnt, anticnt],ShowColorbar=False,
    ...
    _ = mlpf.DrawSnapshot(EqmDf_,fname=f"{SaveFigPath}/unstrained_GaAsPSb.eps", savefig=1, scale=100,
